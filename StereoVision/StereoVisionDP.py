...
def mse(leftDisp,rightDisp,occ):
    leftGroundTruthImg = cv2.imread("/home/infinity/Desktop/Data/disp1.png",0)
    leftGroundTruthImg = np.asarray(leftGroundTruthImg, dtype=np.uint8) 
    rightGroundTruthImg = cv2.imread("/home/infinity/Desktop/Data/disp5.png",0)
    ...
    mseValueLeft=0 #= np.sum(np.square(np.subtract(leftGroundTruthImg, leftDisp)))
    mseValueRight=0 #= np.sum(np.square(np.subtract(rightGroundTruthImg, rightDisp)))
...
def stereoMatching(leftImg,rightImg):
    ...
    # Set occulation value here 
        ...
        # Uncomment the following code to write the left and right disparity images
        
        #cv2.imwrite("Left_Disparity.png",leftDisp)
        #cv2.imwrite("Right_Disparity.png",rightDisp)
...
def main():
    ...
    # Read images.
    leftImg = cv2.imread("/home/infinity/Desktop/Data/view1.png",0)
    leftImg = np.asarray(leftImg, dtype=np.uint8) 
    rightImg = cv2.imread("/home/infinity/Desktop/Data/view5.png",0)
